...
func (runner *Runner) handleInboundPortsInclude() {
	...
	if runner.cfg.InboundPortsInclude != "" {
		if runner.cfg.InboundInterceptionMode == constants.TPROXY {
			...
			runner.iptables.NewChainV4(constants.ISTIODIVERT, constants.MANGLE)
			runner.iptables.AppendRuleV4(constants.ISTIODIVERT, constants.MANGLE, "-j", constants.MARK, "--set-mark", runner.cfg.InboundTProxyMark)
			runner.iptables.AppendRuleV4(constants.ISTIODIVERT, constants.MANGLE, "-j", constants.ACCEPT)
			...
			runner.iptables.NewChainV4(constants.ISTIOTPROXY, constants.MANGLE)
			runner.iptables.AppendRuleV4(constants.ISTIOTPROXY, constants.MANGLE, "!", "-d", "127.0.0.1/32", "-p", constants.TCP, "-j", constants.TPROXY,
				"--tproxy-mark", runner.cfg.InboundTProxyMark+"/0xffffffff", "--on-port", runner.cfg.ProxyPort)
			table = constants.MANGLE
		} else {
			table = constants.NAT
		}
		runner.iptables.NewChainV4(constants.ISTIOINBOUND, table)
		runner.iptables.AppendRuleV4(constants.PREROUTING, table, "-p", constants.TCP, "-j", constants.ISTIOINBOUND)

		if runner.cfg.InboundPortsInclude == "*" {
			// Makes sure SSH is not redirected
			runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, table, "-p", constants.TCP, "--dport", "22", "-j", constants.RETURN)
			// Apply any user-specified port exclusions.
			if runner.cfg.InboundPortsExclude != "" {
				for _, port := range split(runner.cfg.InboundPortsExclude) {
					runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, table, "-p", constants.TCP, "--dport", port, "-j", constants.RETURN)
				}
			}
			...
			if runner.cfg.InboundInterceptionMode == constants.TPROXY {
				...
				// loopback interface.
				runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.MANGLE, "-p", constants.TCP, "-m", "socket", "-j", constants.ISTIODIVERT)
				// Otherwise, it's a new connection. Redirect it using TPROXY.
				runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.MANGLE, "-p", constants.TCP, "-j", constants.ISTIOTPROXY)
			} else {
				runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.NAT, "-p", constants.TCP, "-j", constants.ISTIOINREDIRECT)
			}
		} else {
			...
			for _, port := range split(runner.cfg.InboundPortsInclude) {
				if runner.cfg.InboundInterceptionMode == constants.TPROXY {
					runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.MANGLE, "-p", constants.TCP,
						"--dport", port, "-m", "socket", "-j", constants.ISTIODIVERT)
					runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.MANGLE, "-p", constants.TCP, "--dport", port, "-m",
						"socket", "-j", constants.ISTIODIVERT)
					runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.MANGLE, "-p", constants.TCP, "--dport", port, "-j", constants.ISTIOTPROXY)
				} else {
					runner.iptables.AppendRuleV4(constants.ISTIOINBOUND, constants.NAT, "-p", constants.TCP, "--dport", port, "-j", constants.ISTIOINREDIRECT)
				}
			}
		}
	}
...
func (runner *Runner) handleInboundIpv6Rules(ipv6RangesExclude NetworkRange, ipv6RangesInclude NetworkRange) {
	...
	if runner.cfg.EnableInboundIPv6s != nil {
		...
		// redirects to Envoy.
		runner.iptables.NewChainV6(constants.ISTIOREDIRECT, constants.NAT)
		runner.iptables.AppendRuleV6(constants.ISTIOREDIRECT, constants.NAT, "-p", constants.TCP, "-j", constants.REDIRECT, "--to-port", runner.cfg.ProxyPort)
		// Use this chain also for redirecting inbound traffic to the common Envoy port
		// when not using TPROXY.
		runner.iptables.NewChainV6(constants.ISTIOINREDIRECT, constants.NAT)
		if runner.cfg.InboundPortsInclude == "*" {
			runner.iptables.AppendRuleV6(constants.ISTIOINREDIRECT, constants.NAT, "-p", constants.TCP, "-j",
		...
		if runner.cfg.InboundPortsInclude != "" {
			table = constants.NAT
			runner.iptables.NewChainV6(constants.ISTIOINBOUND, table)
			runner.iptables.AppendRuleV6(constants.PREROUTING, table, "-p", constants.TCP, "-j", constants.ISTIOINBOUND)

			if runner.cfg.InboundPortsInclude == "*" {
				// Makes sure SSH is not redirected
				runner.iptables.AppendRuleV6(constants.ISTIOINBOUND, table, "-p", constants.TCP, "--dport", "22", "-j", constants.RETURN)
				// Apply any user-specified port exclusions.
				if runner.cfg.InboundPortsExclude != "" {
					for _, port := range split(runner.cfg.InboundPortsExclude) {
						runner.iptables.AppendRuleV6(constants.ISTIOINBOUND, table, "-p", constants.TCP, "--dport", port, "-j", constants.RETURN)
					}
				}
			} else {
				// User has specified a non-empty list of ports to be redirunner.cfgted to Envoy.
				for _, port := range split(runner.cfg.InboundPortsInclude) {
					runner.iptables.AppendRuleV6(constants.ISTIOINBOUND, constants.NAT, "-p", constants.TCP, "--dport", port, "-j", constants.ISTIOINREDIRECT)
				}
			}
		}
		// Create a new chain for selectively redirecting outbound packets to Envoy.
		runner.iptables.NewChainV6(constants.ISTIOOUTPUT, constants.NAT)
		// Jump to the ISTIOOUTPUT chain from OUTPUT chain for all tcp traffic.
		runner.iptables.AppendRuleV6(constants.OUTPUT, constants.NAT, "-p", constants.TCP, "-j", constants.ISTIOOUTPUT)
		...
		if runner.cfg.OutboundPortsExclude != "" {
			for _, port := range split(runner.cfg.OutboundPortsExclude) {
				runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-p", constants.TCP, "--dport", port, "-j", constants.RETURN)
			}
		}

		// ::6 is bind connect from inbound passthrough cluster
		runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-o", "lo", "-s", "::6/128", "-j", constants.RETURN)

		// Redirect app calls to back itself via Envoy when using the service VIP or endpoint
		// address, e.g. appN => Envoy (client) => Envoy (server) => appN.
		runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-o", "lo", "!", "-d", "::1/128", "-j", constants.ISTIOINREDIRECT)

		for _, uid := range split(runner.cfg.ProxyUID) {
			// Avoid infinite loops. Don't redirect Envoy traffic directly back to
			// Envoy for non-loopback traffic.
			runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-m", "owner", "--uid-owner", uid, "-j", constants.RETURN)
		}

		for _, gid := range split(runner.cfg.ProxyGID) {
			// Avoid infinite loops. Don't redirunner.cfgt Envoy traffic directly back to
			// Envoy for non-loopback traffic.
			runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-m", "owner", "--gid-owner", gid, "-j", constants.RETURN)
		}
		// Skip redirection for Envoy-aware applications and
		// container-to-container traffic both of which explicitly use
		// localhost.
		runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-d", "::1/128", "-j", constants.RETURN)
		// Apply outbound IPv6 exclusions. Must be applied before inclusions.
		for _, cidr := range ipv6RangesExclude.IPNets {
			runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-d", cidr.String(), "-j", constants.RETURN)
		}
		// Apply outbound IPv6 inclusions.
		if ipv6RangesInclude.IsWildcard {
			// Wildcard specified. Redirect all remaining outbound traffic to Envoy.
			runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-j", constants.ISTIOREDIRECT)
			for _, internalInterface := range split(runner.cfg.KubevirtInterfaces) {
				runner.iptables.InsertRuleV6(constants.PREROUTING, constants.NAT, 1, "-i", internalInterface, "-j", constants.RETURN)
			}
		} else if len(ipv6RangesInclude.IPNets) > 0 {
			// User has specified a non-empty list of cidrs to be redirected to Envoy
			for _, cidr := range ipv6RangesInclude.IPNets {
				for _, internalInterface := range split(runner.cfg.KubevirtInterfaces) {
					runner.iptables.InsertRuleV6(constants.PREROUTING, constants.NAT, 1, "-i", internalInterface,
						"-d", cidr.String(), "-j", constants.ISTIOREDIRECT)
				}
				runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-d", cidr.String(), "-j", constants.ISTIOREDIRECT)
			}
			// All other traffic is not redirected.
			runner.iptables.AppendRuleV6(constants.ISTIOOUTPUT, constants.NAT, "-j", constants.RETURN)
		}
	} else {
		// Drop all inbound traffic except established connections.
		//TODO:
		runner.ext.RunQuietlyAndIgnore(dep.IP6TABLES, "-F", constants.INPUT)
		//TODO: these next 3 commands.. looks like default table: filter
...
func (runner *Runner) handleInboundIpv4Rules(ipv4RangesInclude NetworkRange) {
	...
	if ipv4RangesInclude.IsWildcard {
		// Wildcard specified. Redirect all remaining outbound traffic to Envoy.
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-j", constants.ISTIOREDIRECT)
		for _, internalInterface := range split(runner.cfg.KubevirtInterfaces) {
			runner.iptables.InsertRuleV4(constants.PREROUTING, constants.NAT, 1, "-i", internalInterface, "-j", constants.ISTIOREDIRECT)
		}
		...
		for _, cidr := range ipv4RangesInclude.IPNets {
			for _, internalInterface := range split(runner.cfg.KubevirtInterfaces) {
				runner.iptables.InsertRuleV4(constants.PREROUTING, constants.NAT, 1, "-i", internalInterface,
					"-d", cidr.String(), "-j", constants.ISTIOREDIRECT)
			}
			runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-d", cidr.String(), "-j", constants.ISTIOREDIRECT)
		}
		// All other traffic is not redirected.
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-j", constants.RETURN)
	}
}
...
func (runner *Runner) run() {
	...
	// In both chains, '-j RETURN' bypasses Envoy and '-j ISTIOREDIRECT'
	// redirects to Envoy.
	runner.iptables.NewChainV4(constants.ISTIOREDIRECT, constants.NAT)
	runner.iptables.AppendRuleV4(constants.ISTIOREDIRECT, constants.NAT, "-p", constants.TCP, "-j", constants.REDIRECT, "--to-port", runner.cfg.ProxyPort)
	// Use this chain also for redirecting inbound traffic to the common Envoy port
	// when not using TPROXY.
	runner.iptables.NewChainV4(constants.ISTIOINREDIRECT, constants.NAT)

	// PROXY_INBOUND_CAPTURE_PORT should be used only user explicitly set INBOUND_PORTS_INCLUDE to capture all
	if runner.cfg.InboundPortsInclude == "*" {
		runner.iptables.AppendRuleV4(constants.ISTIOINREDIRECT, constants.NAT, "-p", constants.TCP, "-j", constants.REDIRECT,
	...
	// Create a new chain for selectively redirecting outbound packets to Envoy.
	runner.iptables.NewChainV4(constants.ISTIOOUTPUT, constants.NAT)
	// Jump to the ISTIOOUTPUT chain from OUTPUT chain for all tcp traffic.
	runner.iptables.AppendRuleV4(constants.OUTPUT, constants.NAT, "-p", constants.TCP, "-j", constants.ISTIOOUTPUT)
	// Apply port based exclusions. Must be applied before connections back to self are redirected.
	if runner.cfg.OutboundPortsExclude != "" {
		for _, port := range split(runner.cfg.OutboundPortsExclude) {
			runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-p", constants.TCP, "--dport", port, "-j", constants.RETURN)
		}
	}

	// 127.0.0.6 is bind connect from inbound passthrough cluster
	runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-o", "lo", "-s", "127.0.0.6/32", "-j", constants.RETURN)
	...
	if env.RegisterStringVar("DISABLE_REDIRECTION_ON_LOCAL_LOOPBACK", "", "").Get() == "" {
		...
		// address, e.g. appN => Envoy (client) => Envoy (server) => appN.
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-o", "lo", "!", "-d", "127.0.0.1/32", "-j", constants.ISTIOINREDIRECT)
	}

	for _, uid := range split(runner.cfg.ProxyUID) {
		// Avoid infinite loops. Don't redirect Envoy traffic directly back to
		// Envoy for non-loopback traffic.
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-m", "owner", "--uid-owner", uid, "-j", constants.RETURN)
	}

	for _, gid := range split(runner.cfg.ProxyGID) {
		// Avoid infinite loops. Don't redirect Envoy traffic directly back to
		// Envoy for non-loopback traffic.
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-m", "owner", "--gid-owner", gid, "-j", constants.RETURN)
	}
	// Skip redirection for Envoy-aware applications and
	// container-to-container traffic both of which explicitly use
	// localhost.
	runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-d", "127.0.0.1/32", "-j", constants.RETURN)
	// Apply outbound IPv4 exclusions. Must be applied before inclusions.
	for _, cidr := range ipv4RangesExclude.IPNets {
		runner.iptables.AppendRuleV4(constants.ISTIOOUTPUT, constants.NAT, "-d", cidr.String(), "-j", constants.RETURN)
	}

	for _, internalInterface := range split(runner.cfg.KubevirtInterfaces) {
		runner.iptables.InsertRuleV4(constants.PREROUTING, constants.NAT, 1, "-i", internalInterface, "-j", constants.RETURN)
	}
	...
	for _, cmd := range runner.iptables.BuildV4() {
		runner.ext.RunOrFail("", cmd)
	}

	for _, cmd := range runner.iptables.BuildV6() {
		runner.ext.RunOrFail("", cmd)
	}
}
